...
const convertLeave = (leave: any): Leave => ({
  ...
  approver_id: leave.approver_id ? Number(leave.approver_id) : null,
  request_timestamp: leave.request_timestamp ?? leave.submitted_at ?? new Date().toISOString(),
});
...
const convertShift = (shift: any): Shift => ({
  ...
  status: shift.shift_status === 'unassigned' ? 'Unassigned' : 'Assigned',
...
const OwnerDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (employeesError) {
        ...
        throw new SupabaseError('Failed to fetch employees', 'QUERY_ERROR', employeesError);
      }
      setEmployees((employeesData || []).map(convertEmployee));
      ...
      if (leavesError) {
        ...
        throw new SupabaseError('Failed to fetch leave requests', 'QUERY_ERROR', leavesError);
      }
      setLeaves(Array.isArray(leavesData) ? leavesData.map(convertLeave) : []);
      setPendingLeaveCount(
        Array.isArray(leavesData)
          ? leavesData.reduce((count, leave) => (leave as any !== null && typeof leave === 'object' && 'status' in (leave as any) && (leave as any).status === 'pending') ? count + 1 : count, 0)
          : 0
      );
      ...
      if (shiftsError) {
        ...
        throw new SupabaseError('Failed to fetch shifts', 'QUERY_ERROR', shiftsError);
      }
      setShifts(Array.isArray(shiftsData) ? shiftsData.map(convertShift) : []);
      setUnassignedShiftCount(
        Array.isArray(shiftsData)
          ? shiftsData.reduce((count, shift) => (shift !== null && (shift as any) !== null && typeof shift === 'object' && 'shift_status' in (shift as any) && (shift as any).shift_status === 'unassigned') ? count + 1 : count, 0)
          : 0
      );
  ...
  const handleLeaveStatusChange = async (leaveId: number, newStatus: 'approved' | 'rejected') => {
    try {
      ...
      if (error) {
        ...
        throw new SupabaseError('Failed to update leave status', 'UPDATE_ERROR', error);
      }
    ...
    <div className="container mx-auto p-4 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center">
          <span className="mr-2 text-primary">Welcome, {user?.name}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="border border-[#261e67] rounded-lg bg-[#f2fdff] p-4 shadow-sm">
          <Tabs defaultValue="leave-requests">
            ...
            <TabsContent value="employees" className="space-y-4">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 mb-2">
                <h3 className="font-semibold text-[#001140]">Employee Management</h3>
                <CustomButton variant="secondary" size="sm">Add Employee</CustomButton>
              </div>

              <div className="border border-[#261e67] rounded-lg divide-y divide-[#261e67]">
                {employees.map(employee => (
                  <div key={employee.user_id} className="p-3 flex flex-col md:flex-row justify-between items-start md:items-center gap-2 hover:bg-[#e6f2f9]">
                    <div>
                      <p className="font-medium text-[#001140]">{employee.name}</p>
                      <p className="text-sm text-[#6f7d7f]">{employee.email}</p>
                    </div>
                    <div className="flex items-center">
                      <span className="text-sm mr-2 text-[#001140]">Priority: {employee.priority_level}</span>
